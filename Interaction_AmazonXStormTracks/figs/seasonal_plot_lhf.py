...
# Carrega os dados
df = pd.read_csv('data_lhf.txt', sep='\t', skiprows=1, names=['date', 'tracks_anom', 'wv_anom'])
...
for i, season in enumerate(seasons):
    ...
    ax.bar(data['year'], data['wv_anom'], color='gray', alpha=0.6, label='Fluxo de calor latente (barra)')
...
# Salva a figura
plt.savefig('anomalias_sazonais_ciclones_fluxo_calor_latente_correlacao.png', dpi=300, bbox_inches='tight')
plt.show()
